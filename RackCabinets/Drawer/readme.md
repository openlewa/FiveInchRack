...
Optional **USB SD Holder by openlewa** (`USB_SD_Holder = yes`): upright holders on **one** long edge, packed tight. **SD + USB-A + USB-C** (90°, rounded) overlap in the center; **microSD** (90°) is centered between each SD edge and USB edge. Flat face with connector openings only. Set `Holder_count` to override auto-fit.
