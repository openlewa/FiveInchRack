...
Optional **USB SD Holder by openlewa** (`USB_SD_Holder = yes`): upright holders on both long edges. **SD + USB-A + USB-C** (90°, rounded) overlap in the center; **microSD** (90°) is centered between each SD edge and USB edge. Flat face with connector openings only (no stick-body relief); outer rim beside microSD is thickened. Set `Holder_count` to override auto-fit.
