...
Optional **USB SD Holder by openlewa** (`USB_SD_Holder = yes`): upright holders on both long edges. **SD + USB-A + USB-C** (90°, rounded) overlap in the center; **microSD** (90°) is centered between each SD edge and USB edge, and height-centered with that group. Set `Holder_count` to override auto-fit.
