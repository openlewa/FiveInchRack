include <../fiveinch_library.scad>
  ...
  /////////////////////////////////////////
 // Orange Pi with chassis fan (5" bay) //
/////////////////////////////////////////
// Adapted from pgrondek/SixInchRack OrangePi/orangepi_chassisfan.scad
// Coordinates shifted for the narrower 90mm bay (130mm front plate).
...
if(print_frontplate_sml){ // Frontplate //
    square_hole        = [];
    round_hole         = [[75 ,7.5, 2.1],[79 ,7.5, 2.1],[95, unit,dia-2.0]];
    round_peg          = [];
    screw_side_front   = [];
    screw_top          = [36];
...
if(print_rear){  // Back plate //
    square_hole  = [[57 ,3, 17, 15] , [76 ,4, 6, 14.5]  ];
    round_hole   = [[33,5,5],[33,1.5,5] ];
    screw_top    = [36];//mm
    screw_bottom = [26];//mm
    screw_side   = [];
    depth        = 80;
...
if(print_cabinet_sml){ // Cabinet fan //
    shift = 20;
    fx = 24; // shifted from 36 for narrower bay

    difference(){
        translate([fx,14,69-shift]){
            rotate([0,0,55]){
...
if(print_antennae){
    len = 13;
    ax = 88; // antenna boss X, kept inside 130mm plate
